...
MOXII_API_KEY=your_moxii_api_key_here
```
...
- Verify `MOXII_API_BASE_URL` and `MOXII_API_KEY` are set correctly in Kinde environment variables
...
- Ensure variable names match exactly: `MOXII_API_BASE_URL` and `MOXII_API_KEY`
- Mark the `MOXII_API_KEY` as a secret in Kinde dashboard
